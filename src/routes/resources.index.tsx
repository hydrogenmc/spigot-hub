...
function ResourcesPage() {
  ...
  const activeTags = splitCsv(search.tags);
  const activeDeps = splitCsv(search.deps);
  ...
  const setSearch = (patch: Partial<S>) => navigate({ search: (p: S) => ({ ...p, ...patch }) });
  ...
  const reset = () => navigate({ search: { q: "", category: "", mc: "", tier: "all", tags: "", deps: "", sort: "newest" } });
      ...
      <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 sm:py-14">
        <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
          ...
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            <span className="rounded-full border border-border/60 px-2.5 py-1"><Sparkles size={11} className="mr-1 inline" /> {list.data?.length ?? 0} matching</span>
          </div>
        </div>
        ...
        <div className="glass-strong mt-8 rounded-2xl p-3 sm:p-4">
          <div className="flex flex-col gap-3 md:flex-row md:items-center">
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                value={search.sort}
                onChange={(e) => setSearch({ sort: e.target.value as S["sort"] })}
          ...
          {activeFilterCount > 0 && (
            <div className="mt-3 flex flex-wrap items-center gap-1.5 border-t border-border/40 pt-3">
              ...
              {search.category && <Chip label={`Category: ${search.category}`} onRemove={() => setSearch({ category: "" })} />}
              {search.mc && <Chip label={`MC: ${search.mc}`} onRemove={() => setSearch({ mc: "" })} />}
              {search.tier !== "all" && <Chip label={search.tier === "vip" ? "VIP only" : "Free only"} onRemove={() => setSearch({ tier: "all" })} />}
              {activeTags.map((t) => <Chip key={"t-" + t} label={`#${t}`} onRemove={() => setSearch({ tags: toggleFrom(search.tags, t) })} />)}
              {activeDeps.map((d) => <Chip key={"d-" + d} label={`↳ ${d}`} onRemove={() => setSearch({ deps: toggleFrom(search.deps, d) })} />)}
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[260px_1fr]">
          ...
          <aside className="hidden lg:block">
            <FiltersPanel search={search} setSearch={setSearch} base={base} activeTags={activeTags} activeDeps={activeDeps} toggleFrom={toggleFrom} />
          </aside>
...
function FiltersPanel({ search, setSearch, base, activeTags, activeDeps, toggleFrom }: {
  search: S;
  setSearch: (patch: Partial<S>) => void;
    ...
    <div className="space-y-6">
      <FilterGroup label="Access tier">
        <div className="flex gap-1.5">
          {(["all", "free", "vip"] as const).map((t) => (
            <button key={t} onClick={() => setSearch({ tier: t })}
      ...
      <FilterGroup label="Category">
        <div className="flex flex-wrap gap-1.5">
          <PillBtn active={!search.category} onClick={() => setSearch({ category: "" })}>All</PillBtn>
          {base.categories.map((c) => (
            <PillBtn key={c.id} active={search.category === c.slug} onClick={() => setSearch({ category: c.slug })}>{c.name}</PillBtn>
          ))}
        </div>
      </FilterGroup>
      ...
      <FilterGroup label="Minecraft version">
        <div className="flex flex-wrap gap-1.5">
          <PillBtn active={!search.mc} onClick={() => setSearch({ mc: "" })}>Any</PillBtn>
          {base.facets.versions.slice(0, 12).map((v) => (
            <PillBtn key={v} active={search.mc === v} onClick={() => setSearch({ mc: v })}>{v}</PillBtn>
          ))}
        </div>
        <input value={search.mc} onChange={(e) => setSearch({ mc: e.target.value })} placeholder="Custom (e.g. 1.21)"
        ...
        <FilterGroup label={`Tags ${activeTags.length ? `(${activeTags.length})` : ""}`}>
          <div className="flex max-h-48 flex-wrap gap-1.5 overflow-y-auto pr-1">
            {base.facets.tags.map((t) => (
              <PillBtn key={t} active={activeTags.includes(t)} onClick={() => setSearch({ tags: toggleFrom(search.tags, t) })}>#{t}</PillBtn>
            ))}
          </div>
        </FilterGroup>
      )}
        ...
        <FilterGroup label={`Dependencies ${activeDeps.length ? `(${activeDeps.length})` : ""}`}>
          <div className="flex max-h-48 flex-wrap gap-1.5 overflow-y-auto pr-1">
            {base.facets.dependencies.map((d) => (
              <PillBtn key={d} active={activeDeps.includes(d)} onClick={() => setSearch({ deps: toggleFrom(search.deps, d) })}>{d}</PillBtn>
            ))}
          </div>
        </FilterGroup>
      )}
